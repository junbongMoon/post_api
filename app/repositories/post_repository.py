...
#       여러개의 관계 테이블이 있을 때 트랜잭션 처리가 필요하다 하더라도,
#       repository단에서는 commit과 rollback을 절대 하지 않는다.!!!
#       commit과 rollback은 트랜잭션의 컨트롤 타워인 [Service단]에서 담당한다.
#       Service단에서 트랜잭션을 컨트롤하기 위해서, flush()는 해야함.
...
# 관계 있는 테이블을 미리 join으로 함께 가져올 때 사용한다.
...
class PostRepository : 
  ...
  def create_post_tx(self, title:str, content:str, author:str) -> Post :
    ...
    self.db.add(post) # SqlAlchemy세션에 pst 객체 등록
    # insert sql이 db에 전달 되어 실행되어 id가 생성 된다.
    self.db.flush() # post.id를 사용할 수 있게 됨.
  ...
  def get_with_relations(self, post_id:int) -> Optional[Post]:
    ...
    return (
      ...
      .options(
        joinedload(Post.stat),  # post_stat 테이블과의 조인 (조건도 포함되어 있다.)
        joinedload(Post.attachments)  # attachment 테이블과의 조인
      )
      .filter(Post.id == post_id) # where절 (넘겨져 온 게시글의 번호)
      .first()
    )
  ...
  def insert(self, title:str, content:str, author:str) -> Post :
    ...
    """
    print("게시글을 실제 디비에 저장하자! 여기는 레포지토리 단!!!!")
    post = Post(
      title=title,
      content=content,
      author=author,
    ...
    self.db.add(post) # insert 쿼리문 실행
  ...
  def get_post_list(self, offset:int = 0, limit:int = 10, search:Optional[str] = None, author:Optional[str] = None, order_by:str = "latest") -> List[Post] :
    """
      게시글 전체 목록 반환
    ...
    if query.order_by == "views" : # 정렬 조건이 조회수 순일 때
  ...
  def get_post_count(self, search:Optional[str] = None, author:Optional[str] = None) -> int:
    ...
    query = self.db.query(func.count(Post.id))  # 전체 글의 수
