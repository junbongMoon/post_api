...
from app.util.paging import calc_paging_block
...
class PostService :
  ...
  def get_list(self, page:int, per_page:int, 
    ...
    return PostListResponse(
      ...
      page_info=PagingInfo.model_validate(calc_paging_block(page_no=page, page_size=per_page, total_post_cnt=count))
      )
  ...
  def update_post(self, id:int, data:PostUpdate) -> PostDetail:
    ...
    post = self.repo.update(post=post, changes=changes)
    return PostDetail.model_validate(post)
  ...
  def create_post_with_attachments(
    ...
    try :
      ...
      self.repo.create_stat(post_id=post.id)      
      filenames = [att.filename for att in data.attachments]      
      self.repo.create_attachments(post_id=post.id,filenames=filenames)
